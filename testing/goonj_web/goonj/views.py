...
from goonj.models import Campaigns, CampaignStoryRelation, Timeline, TimelineStoryRelation, Story,Issue,CommunityRep,GovernmentDepartment,ChannelPartner,TopicChannel,Location,Photo,Loc_country,Loc_state,Loc_district,Loc_block,Loc_village
...
def timelines(request): 	#FOR TESTING ONLY
	stories = []
	publish_dates = []
	events = Timeline.objects.filter(status = 'active') #choose active ones only
	events = list(events)
	for t in events:
	...
	for i in xrange(0, len(publish_dates)-1):	#Rearrange timelines chronologically
		temp_date = publish_dates[i]
		temp_events = events[i]
		temp_stories = stories[i]
		if publish_dates[i+1] > publish_dates[i] :
			publish_dates[i] = publish_dates[i+1]
			events[i] = events[i+1]
			stories[i] = stories[i+1]
			publish_dates[i+1] = temp_date
			events[i+1] = temp_events
			stories[i+1] = temp_stories	
	return render_to_response('goonj/events.html', {'events' : events, 'stories_list' :stories, 'publish_dates' : publish_dates})
	
def campaigns(request):
	stories = []
	publish_dates = []
	campaigns = Campaigns.objects.filter(status = 'active') #choose active ones only
	campaigns = list(campaigns)		#Change queryset to a list
	for c in campaigns:
		...
		stories_for_event = Story.objects.filter(story_id__in = list_of_ids).order_by('-date')
		stories.append(stories_for_event)	#nth element is list of stories related to nth event
	for story_list in stories:					# Find date of latest added story
		dates = []
		for story in story_list:
			dates.append(story.date)
		publish_dates.append(max(dates))
	for i in xrange(0, len(publish_dates)-1):	#Rearrange campaigns chronologically
		temp_date = publish_dates[i]
		temp_campaigns = campaigns[i]
		temp_stories = stories[i]
		if publish_dates[i+1] > publish_dates[i] :
			publish_dates[i] = publish_dates[i+1]
			...
			stories[i] = stories[i+1]
			publish_dates[i+1] = temp_date
			campaigns[i+1] = temp_campaigns
			stories[i+1] = temp_stories
	return render_to_response('goonj/campaigns.html', {'campaigns' : campaigns, 'stories_list' : stories, 'publish_dates' : publish_dates})	
